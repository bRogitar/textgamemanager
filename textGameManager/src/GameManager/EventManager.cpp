...
std::unique_ptr<Event> EventManager::loadEventFromXML(const std::string& filePath, const std::string& eventId) {
    ...
    while (pEventElement != nullptr) {
        if (std::string(pEventElement->Attribute("id")) == eventId) {
            ...
            // 트랩 정보를 읽어들임
            XMLElement* pTrapElement = pEventElement->FirstChildElement("Trap");
            if (pTrapElement != nullptr) {
                const char* trapType = pTrapElement->Attribute("type");
                int damage = 0, avoidChance = 0;
                pTrapElement->QueryIntAttribute("damage", &damage);
                pTrapElement->QueryIntAttribute("avoidChance", &avoidChance);
                
                if (strcmp(trapType, "PoisonGas") == 0) {
                    newEvent->setTrap(std::make_unique<PoisonGasTrap>(damage, avoidChance));
                }
            }

            // NPC 정보를 읽어들임
            XMLElement* pNPCElement = pEventElement->FirstChildElement("NPC");
            if (pNPCElement != nullptr) {
                const char* npcType = pNPCElement->Attribute("type");
                if (strcmp(npcType, "Shopkeeper") == 0) {
                    const char* npcId = pNPCElement->Attribute("id");
                    const char* npcName = pNPCElement->Attribute("name");
                    const char* dialogue = pNPCElement->Attribute("dialogue");
                    ...
                    newEvent->setNPC(std::move(shopkeeper));
                }
            }
