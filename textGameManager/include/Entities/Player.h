...
public:
    ...
    // 능력 적용 메서드 (추가됨)
    void applyAbilities();
    void clearAbilities() { //어빌리티 초기화
        abilityInventory.clearAbilities();
    }

    // AbilityInventory getter 추가
    const AbilityInventory& getAbilityInventory() const {
...
private:
    ...
    static constexpr int DEFAULT_ATTACK_POWER = 20;
